...
from slaif_asr.trainable_surface_sweep import (
    ...
    SURFACE06_ENCODER_BLOCK_PREFIXES,
    SURFACE06_ID,
...
DEFAULT_CONFIG = Path("configs/experiments/fixed-scale2000-surface06-last-four-encoder-blocks.json")
ARM_NAME = "fixed_scale2000_surface06_last_four_encoder_blocks"
FAST_DIRECTIONAL_CONFIG = Path("configs/experiments/fast_batched_directional_replay_v1.json")
REPORT_JSON = Path("docs/experiments/0026-fixed-scale2000-surface06-last-four-encoder-blocks.json")
REPORT_MD = Path("docs/experiments/0026-fixed-scale2000-surface06-last-four-encoder-blocks.md")
CERTIFICATE_PATH = Path("docs/data-certificates/sl-corpus-v4-fixed-scale2000-surface06-diagnostic-v1.json")
...
_BASE_SPEC = importlib.util.spec_from_file_location("_slaif_surface06_model_restore", _BASE_PATH)
...
def _assert_gradient_scope(model: Any) -> None:
    bad = [
        ...
        if parameter.grad is not None and not name.startswith(SURFACE06_ALLOWED_TRAINABLE_PREFIXES)
    ]
    if bad:
...
def fingerprint_integrity(before: dict[str, str], after: dict[str, str]) -> dict[str, Any]:
    ...
    return surface06_changed_tensor_summary(
...
def stage_verify_inputs(config_path: Path) -> dict[str, Any]:
    config = load_surface06_config(config_path)
    payload = {"status": "PASSED", "work_order_id": "0039", **verify_all_inputs(config)}
...
def _run_gradient_partition(config: dict[str, Any], records: Sequence[Any], physical: int, torch: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    configure_surface06_trainable(model)
...
def stage_probe_microbatch(config_path: Path, interval: float) -> dict[str, Any]:
    ...
    reporter.start("probing SURFACE_06 microbatch")
    for candidate in config["training"]["physical_microbatch_candidates"]:
        ...
        try:
            ...
            model = restore_base_model(config)
            configure_surface06_trainable(model)
            set_surface06_training_mode(model)
            prompt = derive_prompt_column_selection(model, "sl-SI")
            ...
            gc.collect()
            torch.cuda.empty_cache()
    selected = select_surface06_microbatch(outcomes)
    if selected["status"] != "PASSED":
        ...
        raise RuntimeError("SURFACE_06 does not fit physical microbatch 1")
...
def _load_or_initialize_training(
    ...
    if latest is None:
        model = restore_base_model(config, reporter=reporter)
        surface = configure_surface06_trainable(model)
        set_surface06_training_mode(model)
        optimizer = torch.optim.AdamW(surface06_optimizer_parameter_groups(model, rates), weight_decay=0.0)
        verify_surface06_optimizer_scope(optimizer, model, rates)
...
def stage_train(config_path: Path, interval: float) -> dict[str, Any]:
    ...
    reporter.start("training fixed scale-2000 SURFACE_06")
    ...
    try:
        for round_index in range(resume_round + 1, int(config["training"]["max_rounds"]) + 1):
            ...
            for batch_indices in layout.batches:
                ...
                for start in range(0, 8, physical):
                    ...
                    loss = rnnt_audio_loss(model, batch, prompt.prompt_index, frozen_encoder_no_grad=False)
                    if not torch.isfinite(loss):
                        raise RuntimeError("non-finite SURFACE_06 RNNT loss")
                ...
                if not finite:
                    raise RuntimeError("non-finite SURFACE_06 gradient")
                optimizer.step()
                optimizer_steps += 1
                exposures_seen += 8
                segment_exposures += 8
            ...
            norm_row = {
                ...
                "final_four_encoder_blocks_norm": round(
                    sum(
                        ...
                        for name, parameter in model.named_parameters()
                        if name.startswith(SURFACE06_ENCODER_BLOCK_PREFIXES)
                    ),
                    6,
                ),
            }
        ...
        reporter.failed("SURFACE_06 training failed", error_type=type(exc).__name__)
        raise
    finally:
        monitor.stop()
    ...
    if not integrity["only_surface06_changed"]:
    ...
    groups = {
        ...
        "encoder_final_four_blocks": [
            name for name in initial_fingerprints if name.startswith(SURFACE06_ENCODER_BLOCK_PREFIXES)
        ],
        "encoder_lower_frozen": [
            name
            for name in initial_fingerprints
            if name.startswith("encoder.layers.") and not name.startswith(SURFACE06_ENCODER_BLOCK_PREFIXES)
        ],
    ...
    for name, tensor_names in groups.items():
        ...
        expected = "trainable" if name in {"decoder", "joint", "encoder_final_four_blocks"} else "frozen"
    ...
    reporter.complete("SURFACE_06 training and controller selection complete", step=optimizer_steps, total_steps=40000)
...
def stage_evaluate_directional(config_path: Path) -> dict[str, Any]:
    ...
    classification = classify_surface06(
        metric_table,
        parameter_integrity=bool(training["parameter_integrity"]["only_surface06_changed"]),
        selected_round=selected_round,
    )
...
def _markdown_report(public: dict[str, Any]) -> str:
    ...
    if public["classification"] == "SURFACE06_NEW_BEST_DIRECTIONAL_CANDIDATE":
        interpretation = "Surface06 is a new best directional candidate; Surface07 remains subject to separate strategic review and work-order approval."
    elif public["classification"] == "SURFACE06_MATCHES_BEST_WITH_ACCEPTABLE_TRADEOFF":
        interpretation = "Surface06 matches the best-known envelope with an acceptable tradeoff; Surface07 remains subject to separate strategic review and work-order approval."
    elif public["classification"] == "SURFACE06_ARTUR_DEV_GOOD_BUT_FLEURS_REGRESSES":
        interpretation = "Surface06 crossed the FLEURS tolerance boundary despite ARTUR improvement; deeper encoder expansion should stop."
    else:
        interpretation = "Surface06 did not justify automatic encoder-depth expansion; Surface07 remains unauthorized pending strategic review."
    lines = [
        ...
        f"- Trainable parameters: {surface['trainable_parameter_count']:,} total; {surface['decoder_parameter_count']:,} decoder, {surface['joint_parameter_count']:,} joint, and {surface['final_four_encoder_blocks_parameter_count']:,} final-four encoder blocks.",
    ...
    for split in ("piper_synthetic_holdout", "supertonic_heldout_voice_holdout", "fleurs_v2", "artur_j"):
        lines.append(
            f"| {split} | {_format_metric(metrics['base'][split])} | {_format_metric(metrics['scale2000_joint_adapter'][split])} | {_format_metric(metrics['pr36_round20'][split])} | {_format_metric(metrics['pr39_round6'][split])} | {_format_metric(metrics['surface04_selected'][split])} | {_format_metric(metrics['surface05_selected'][split])} | {_format_metric(metrics['surface06_selected'][split])} |"
        )
    lines.extend(
        [
            "",
    ...
    for row in public["best_known_real_gate_envelope"]:
        ...
        lines.append(
            f"| {label} | {row['best_prior_value']:.3f} | {row['prior_source']} | {row['surface06_value']:.3f} | {str(row['within_tolerance']).lower()} |"
        )
    lines.extend(
        [
            "",
...
def stage_summarize(config_path: Path) -> dict[str, Any]:
    ...
    evaluation["classification"] = classify_surface06(
    ...
    public = {
        ...
        "directional_evaluation": {
            ...
            "metrics": {
                ...
                "surface06_selected": evaluation["metric_table"],
            },
            "selected_round": training["selected_round"],
            "selected_checkpoint_sha256": evaluation["checkpoint_sha256"],
            "classification": evaluation["classification"],
        },
        "best_known_real_gate_envelope": surface06_envelope_comparison(evaluation["metric_table"]),
        "limitations": [
            ...
            "Surface07 remains unauthorized pending separate strategic review and work-order approval.",
        ],
        "safety": {
    ...
    certificate = {
        "schema_version": "1.0",
        "certificate_id": "sl-corpus-v4-fixed-scale2000-surface06-diagnostic-v1",
        "status": "DIAGNOSTIC_ONLY",
        "work_order_id": "0039",
        "corpus_id": config["data"]["corpus_id"],
        "fixed_text_sha256": config["data"]["fixed_text_sha256"],
        ...
        "selected_checkpoint_sha256": evaluation["checkpoint_sha256"],
        "classification": evaluation["classification"],
        "parameter_integrity_passed": training["parameter_integrity"]["only_surface06_changed"],
        "accepted_parent": "none",
        "promotion_eligible": False,
        "training_eligible": False,
        "checkpoint_accepted": False,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Work Order 0039 Surface06 diagnostic")
