...
DEFAULT_CONFIG = Path("configs/experiments/fixed-scale2000-surface08-full-encoder.json")
ARM_NAME = "fixed_scale2000_surface08_full_encoder"
FAST_DIRECTIONAL_CONFIG = Path("configs/experiments/fast_batched_directional_replay_v1.json")
REPORT_JSON = Path("docs/experiments/0029-fixed-scale2000-surface08-full-encoder.json")
REPORT_MD = Path("docs/experiments/0029-fixed-scale2000-surface08-full-encoder.md")
CERTIFICATE_PATH = Path("docs/data-certificates/sl-corpus-v4-fixed-scale2000-surface08-diagnostic-v1.json")
...
_BASE_SPEC = importlib.util.spec_from_file_location("_slaif_surface08_model_restore", _BASE_PATH)
...
def _assert_gradient_scope(model: Any) -> None:
    bad = [
        ...
        if parameter.grad is not None and not name.startswith(SURFACE08_ALLOWED_TRAINABLE_PREFIXES)
    ]
    if bad:
...
def fingerprint_integrity(before: dict[str, str], after: dict[str, str]) -> dict[str, Any]:
    ...
    return surface08_changed_tensor_summary(
...
def stage_verify_inputs(config_path: Path) -> dict[str, Any]:
    config = load_surface08_config(config_path)
    payload = {"status": "PASSED", "work_order_id": "0043", **verify_all_inputs(config)}
...
def stage_probe_surface(config_path: Path) -> dict[str, Any]:
    ...
    summary = configure_surface08_trainable(model)
    expected = {
        "decoder_parameter_count": 14940160,
        "joint_parameter_count": 9455648,
        "encoder_all_layers_parameter_count": 604545024,
        "fusion_bridge_parameter_count": 4459520,
        "trainable_parameter_count": 633400352,
        "frozen_parameter_count": 4596736,
    }
...
def _memory_probe_records(config: dict[str, Any], model: Any, physical: int) -> list[Any]:
    rounds, _meta, _summary = load_scheduled_round_records(config)
    token_lengths = {
        record.selected_training_id: len(model.tokenizer.text_to_ids(record.text))
        for record in rounds[1]
    }
    worst_records: list[Any] | None = None
    worst_score: tuple[float, float, int, int, int] | None = None
    for round_index in range(1, int(config["training"]["max_rounds"]) + 1):
        ...
        for batch_number, batch_indices in enumerate(layout.batches):
            for start in range(0, 8, physical):
                micro = [records[index] for index in batch_indices[start : start + physical]]
                max_duration = max(record.duration for record in micro)
                max_tokens = max(token_lengths[record.selected_training_id] for record in micro)
                score = (
                    max_duration * (max_tokens + 1) * len(micro),
                    max_duration,
                    max_tokens,
                    -round_index,
                    -batch_number,
                ...
                if worst_score is None or score > worst_score:
                    worst_score = score
                    worst_records = micro
    if worst_records is None:
        raise RuntimeError("could not select a deterministic memory-probe microbatch")
...
def _run_gradient_partition(config: dict[str, Any], records: Sequence[Any], physical: int, torch: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    configure_surface08_trainable(model)
...
def stage_probe_microbatch(config_path: Path, interval: float) -> dict[str, Any]:
    ...
    for candidate in config["training"]["physical_microbatch_candidates"]:
        ...
        try:
            ...
            prompt = derive_prompt_column_selection(model, "sl-SI")
            candidate_records = _memory_probe_records(config, model, physical=int(candidate))
            batch = make_training_batch(model, candidate_records, device="cuda")
            loss = rnnt_audio_loss(model, batch, prompt.prompt_index, frozen_encoder_no_grad=False)
            if not torch.isfinite(loss):
...
def _load_or_initialize_training(
    ...
    if latest is None:
        model = restore_base_model(config, reporter=reporter)
        surface = configure_surface08_trainable(model)
        set_surface08_training_mode(model)
...
def stage_train(config_path: Path, interval: float) -> dict[str, Any]:
    ...
    reporter.start("training fixed scale-2000 SURFACE_08")
    ...
    if not state["probe_curve"]:
        ...
        controller = evaluate_controller_checkpoint(
            config,
            _checkpoint_dir(config, 0) / "model.local.nemo",
            0,
            os.environ.get("SLAIF_VALIDATION_GPU", hardware.physical_selector),
        )
    ...
    validation_gpu = os.environ.get("SLAIF_VALIDATION_GPU", hardware.physical_selector)
    if validation_gpu != hardware.physical_selector:
        raise RuntimeError("Surface08 requires sequential controller validation on the one visible training GPU")
        ...
        for round_index in range(resume_round + 1, int(config["training"]["max_rounds"]) + 1):
            ...
            for batch_indices in layout.batches:
                ...
                for start in range(0, 8, physical):
                    ...
                    loss = rnnt_audio_loss(model, batch, prompt.prompt_index, frozen_encoder_no_grad=False)
                    if not torch.isfinite(loss):
                        raise RuntimeError("non-finite SURFACE_08 RNNT loss")
                ...
                if not finite:
                    raise RuntimeError("non-finite SURFACE_08 gradient")
                optimizer.step()
                optimizer_steps += 1
                exposures_seen += 8
                segment_exposures += 8
            ...
            norm_row = {
                ...
                "encoder_all_layers_norm": round(
                    sum(
                        float(torch.linalg.vector_norm(parameter.detach()).cpu())
                        for name, parameter in model.named_parameters()
                        if name.startswith(SURFACE08_ENCODER_LAYER_PREFIX)
                    ),
                    6,
                ),
                "fusion_bridge_norm": round(
                    sum(
                        float(torch.linalg.vector_norm(parameter.detach()).cpu())
                        for name, parameter in model.named_parameters()
                        if name.startswith(SURFACE08_FUSION_BRIDGE_PREFIX)
                    ),
                    6,
                ),
            }
    ...
    groups = {
        ...
        "encoder_all_layers": [
            name for name in initial_fingerprints if name.startswith(SURFACE08_ENCODER_LAYER_PREFIX)
        ],
        "fusion_bridge_candidate": [
            name for name in initial_fingerprints if name.startswith(SURFACE08_FUSION_BRIDGE_PREFIX)
        ],
    ...
    for name, tensor_names in groups.items():
        ...
        expected = (
            "trainable"
            if name in {"decoder", "joint", "encoder_all_layers", "fusion_bridge_candidate"}
            else "frozen"
        )
    ...
    required_changed_groups = {
        "decoder": "decoder.",
        "joint": "joint.",
        "encoder_all_layers": SURFACE08_ENCODER_LAYER_PREFIX,
        "fusion_bridge_candidate": SURFACE08_FUSION_BRIDGE_PREFIX,
    }
    ...
    unchanged_encoder_layers = [
        index
        for index in range(24)
        if not any(
            tensor_name.startswith(f"encoder.layers.{index}.")
            and initial_fingerprints[tensor_name] != after[tensor_name]
            for tensor_name in initial_fingerprints
        )
    ]
    if unchanged_encoder_layers:
        raise RuntimeError(f"required encoder layers did not change: {unchanged_encoder_layers}")
    payload = {
        "status": "PASSED",
    ...
    reporter.complete("SURFACE_08 training and controller selection complete", step=optimizer_steps, total_steps=40000)
...
def stage_evaluate_directional(config_path: Path) -> dict[str, Any]:
    ...
    classification = classify_surface08(
        metric_table,
        parameter_integrity=bool(training["parameter_integrity"]["only_surface08_changed"]),
        selected_round=selected_round,
    )
...
def _markdown_report(public: dict[str, Any]) -> str:
    ...
    if public["classification"] == "SURFACE08_NEW_BEST_DIRECTIONAL_CANDIDATE":
        interpretation = "Surface08 is a new best directional candidate; it still remains a one-time synthetic-only boundary diagnostic."
    elif public["classification"] == "SURFACE08_MATCHES_SURFACE07_WITH_ACCEPTABLE_TRADEOFF":
        interpretation = "Surface08 matches the Surface07 envelope with an acceptable tradeoff; full-encoder exposure did not establish a generally preferred training policy."
    elif public["classification"] == "SURFACE08_ARTUR_DEV_GOOD_BUT_FLEURS_REGRESSES":
        interpretation = "ARTUR controller-dev selected a Surface08 checkpoint, but FLEURS crossed the Surface07 tolerance boundary; full-encoder expansion should stop."
    else:
        interpretation = "Surface08 did not improve the fixed-data Surface07 envelope; full-encoder expansion should stop."
    lines = [
        ...
        "## Result",
        "",
        f"- Surface: `{surface['surface_id']}` (`{surface['encoder_layers'][0]}` through `{surface['encoder_layers'][-1]}`).",
        ...
        f"- Trainable parameters: {surface['trainable_parameter_count']:,} total; {surface['decoder_parameter_count']:,} decoder, {surface['joint_parameter_count']:,} joint, {surface['encoder_all_layers_parameter_count']:,} encoder layers, and {surface['fusion_bridge_parameter_count']:,} prompt kernel.",
    ...
    for split in ("piper_synthetic_holdout", "supertonic_heldout_voice_holdout", "fleurs_v2", "artur_j"):
        lines.append(
            f"| {split} | {_format_metric(metrics['base'][split])} | {_format_metric(metrics['pr36_round20'][split])} | {_format_metric(metrics['surface06_selected'][split])} | {_format_metric(metrics['surface07_selected'][split])} | {_format_metric(metrics['surface08_selected'][split])} |"
        )
    lines.extend(
        [
            "",
    ...
    for row in public["best_known_real_gate_envelope"]:
        ...
        lines.append(
            f"| {label} | {row['best_prior_value']:.3f} | {row['prior_source']} | {row['surface08_value']:.3f} | {str(row['within_tolerance']).lower()} |"
        )
    lines.extend(
        [
            "",
...
def stage_summarize(config_path: Path) -> dict[str, Any]:
    ...
    evaluation["classification"] = classify_surface08(
    ...
    public = {
        ...
        "directional_evaluation": {
            ...
            "metrics": {
                ...
                "surface07_selected": SURFACE07_METRICS,
                "surface08_selected": evaluation["metric_table"],
            },
            "selected_round": training["selected_round"],
            "selected_checkpoint_sha256": evaluation["checkpoint_sha256"],
            "classification": evaluation["classification"],
        },
        "best_known_real_gate_envelope": surface08_envelope_comparison(evaluation["metric_table"]),
        "limitations": [
            ...
            "Training the full encoder and shared fusion bridge may affect languages not evaluated by this Slovenian-first diagnostic.",
            "Work Order 0043 is a one-time Surface08 boundary exception and does not authorize Surface09 or full-model training.",
        ],
        "safety": {
    ...
    certificate = {
        "schema_version": "1.0",
        "certificate_id": "sl-corpus-v4-fixed-scale2000-surface08-diagnostic-v1",
        "status": "DIAGNOSTIC_ONLY",
        "work_order_id": "0043",
        "corpus_id": config["data"]["corpus_id"],
        "fixed_text_sha256": config["data"]["fixed_text_sha256"],
        ...
        "encoder_all_layers_parameter_count": training["surface"]["encoder_all_layers_parameter_count"],
        "selected_round": training["selected_round"],
        "selected_checkpoint_sha256": evaluation["checkpoint_sha256"],
        "classification": evaluation["classification"],
        "parameter_integrity_passed": training["parameter_integrity"]["only_surface08_changed"],
        "accepted_parent": "none",
        "promotion_eligible": False,
        "training_eligible": False,
        "checkpoint_accepted": False,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Work Order 0043 Surface08 diagnostic")
