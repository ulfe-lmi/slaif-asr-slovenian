...
from slaif_asr.trainable_surface_sweep import (
    ...
    SURFACE05_ENCODER_BLOCK_PREFIXES,
    SURFACE05_ID,
...
DEFAULT_CONFIG = Path("configs/experiments/fixed-scale2000-surface05-last-two-encoder-blocks.json")
ARM_NAME = "fixed_scale2000_surface05_last_two_encoder_blocks"
FAST_DIRECTIONAL_CONFIG = Path("configs/experiments/fast_batched_directional_replay_v1.json")
REPORT_JSON = Path("docs/experiments/0025-fixed-scale2000-surface05-last-two-encoder-blocks.json")
REPORT_MD = Path("docs/experiments/0025-fixed-scale2000-surface05-last-two-encoder-blocks.md")
CERTIFICATE_PATH = Path("docs/data-certificates/sl-corpus-v4-fixed-scale2000-surface05-diagnostic-v1.json")
...
_BASE_SPEC = importlib.util.spec_from_file_location("_slaif_surface05_model_restore", _BASE_PATH)
...
def _assert_gradient_scope(model: Any) -> None:
    bad = [
        ...
        if parameter.grad is not None and not name.startswith(SURFACE05_ALLOWED_TRAINABLE_PREFIXES)
    ]
    if bad:
...
def fingerprint_integrity(before: dict[str, str], after: dict[str, str]) -> dict[str, Any]:
    ...
    return surface05_changed_tensor_summary(
...
def stage_verify_inputs(config_path: Path) -> dict[str, Any]:
    config = load_surface05_config(config_path)
    payload = {"status": "PASSED", "work_order_id": "0038", **verify_all_inputs(config)}
...
def stage_probe_surface(config_path: Path) -> dict[str, Any]:
    ...
    summary = configure_surface05_trainable(model)
    expected = {
        "decoder_parameter_count": 14940160,
        "joint_parameter_count": 9455648,
        "final_two_encoder_blocks_parameter_count": 50378752,
        "trainable_parameter_count": 74774560,
    }
...
def _run_gradient_partition(config: dict[str, Any], records: Sequence[Any], physical: int, torch: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    configure_surface05_trainable(model)
...
def stage_probe_microbatch(config_path: Path, interval: float) -> dict[str, Any]:
    ...
    reporter.start("probing SURFACE_05 microbatch")
    for candidate in config["training"]["physical_microbatch_candidates"]:
        ...
        try:
            ...
            model = restore_base_model(config)
            configure_surface05_trainable(model)
            set_surface05_training_mode(model)
            prompt = derive_prompt_column_selection(model, "sl-SI")
            ...
            gc.collect()
            torch.cuda.empty_cache()
    selected = select_surface05_microbatch(outcomes)
    if selected["status"] != "PASSED":
        ...
        raise RuntimeError("SURFACE_05 does not fit physical microbatch 1")
...
def _load_or_initialize_training(
    ...
    if latest is None:
        model = restore_base_model(config, reporter=reporter)
        surface = configure_surface05_trainable(model)
        set_surface05_training_mode(model)
        optimizer = torch.optim.AdamW(surface05_optimizer_parameter_groups(model, rates), weight_decay=0.0)
        verify_surface05_optimizer_scope(optimizer, model, rates)
...
def stage_train(config_path: Path, interval: float) -> dict[str, Any]:
    ...
    reporter.start("training fixed scale-2000 SURFACE_05")
    ...
    try:
        for round_index in range(resume_round + 1, int(config["training"]["max_rounds"]) + 1):
            ...
            for batch_indices in layout.batches:
                ...
                for start in range(0, 8, physical):
                    ...
                    loss = rnnt_audio_loss(model, batch, prompt.prompt_index, frozen_encoder_no_grad=False)
                    if not torch.isfinite(loss):
                        raise RuntimeError("non-finite SURFACE_05 RNNT loss")
                ...
                if not finite:
                    raise RuntimeError("non-finite SURFACE_05 gradient")
                optimizer.step()
                optimizer_steps += 1
                exposures_seen += 8
                segment_exposures += 8
            ...
            norm_row = {
                ...
                "final_two_encoder_blocks_norm": round(
                    sum(
                        float(torch.linalg.vector_norm(parameter.detach()).cpu())
                        for name, parameter in model.named_parameters()
                        if name.startswith(SURFACE05_ENCODER_BLOCK_PREFIXES)
                    ),
                    6,
                ),
            }
        ...
        reporter.failed("SURFACE_05 training failed", error_type=type(exc).__name__)
        raise
    finally:
        monitor.stop()
    ...
    if not integrity["only_surface05_changed"]:
    ...
    groups = {
        ...
        "encoder_final_two_blocks": [
            name for name in initial_fingerprints if name.startswith(SURFACE05_ENCODER_BLOCK_PREFIXES)
        ],
        "encoder_lower_frozen": [
            name
            for name in initial_fingerprints
            if name.startswith("encoder.layers.") and not name.startswith(SURFACE05_ENCODER_BLOCK_PREFIXES)
        ],
    ...
    for name, tensor_names in groups.items():
        ...
        expected = "trainable" if name in {"decoder", "joint", "encoder_final_two_blocks"} else "frozen"
    ...
    reporter.complete("SURFACE_05 training and controller selection complete", step=optimizer_steps, total_steps=40000)
...
def stage_evaluate_directional(config_path: Path) -> dict[str, Any]:
    ...
    classification = classify_surface05(
        metric_table,
        parameter_integrity=bool(training["parameter_integrity"]["only_surface05_changed"]),
        selected_round=selected_round,
    )
...
def _markdown_report(public: dict[str, Any]) -> str:
    ...
    if public["classification"] == "SURFACE05_NEW_BEST_DIRECTIONAL_CANDIDATE":
        interpretation = "Surface05 is a new best directional candidate, but Surface06 remains subject to separate strategic review."
    elif public["classification"] == "SURFACE05_MATCHES_BEST_WITH_ACCEPTABLE_TRADEOFF":
        interpretation = "Surface05 matches the best-known envelope with an acceptable tradeoff; Surface06 remains subject to separate strategic review."
    else:
        interpretation = "Surface05 did not justify automatic encoder-depth expansion; Surface06 remains unauthorized pending strategic review."
    lines = [
        ...
        f"- Trainable parameters: {surface['trainable_parameter_count']:,} total; {surface['decoder_parameter_count']:,} decoder, {surface['joint_parameter_count']:,} joint, and {surface['final_two_encoder_blocks_parameter_count']:,} final-two encoder blocks.",
    ...
    for split in ("piper_synthetic_holdout", "supertonic_heldout_voice_holdout", "fleurs_v2", "artur_j"):
        lines.append(
            f"| {split} | {_format_metric(metrics['base'][split])} | {_format_metric(metrics['scale2000_joint_adapter'][split])} | {_format_metric(metrics['pr36_round20'][split])} | {_format_metric(metrics['pr39_round6'][split])} | {_format_metric(metrics['surface04_selected'][split])} | {_format_metric(metrics['surface05_selected'][split])} |"
        )
    lines.extend(
        [
            "",
    ...
    for row in public["best_known_real_gate_envelope"]:
        ...
        lines.append(
            f"| {label} | {row['best_prior_value']:.3f} | {row['prior_source']} | {row['surface05_value']:.3f} | {str(row['within_tolerance']).lower()} |"
        )
    lines.extend(
        [
            "",
...
def stage_summarize(config_path: Path) -> dict[str, Any]:
    ...
    evaluation["classification"] = classify_surface05(
    ...
    public = {
        ...
        "directional_evaluation": {
            ...
            "metrics": {
                ...
                "surface05_selected": evaluation["metric_table"],
            },
            "selected_round": training["selected_round"],
            "selected_checkpoint_sha256": evaluation["checkpoint_sha256"],
            "classification": evaluation["classification"],
        },
        "best_known_real_gate_envelope": surface05_envelope_comparison(evaluation["metric_table"]),
        "limitations": [
            ...
            "Surface06 remains unauthorized pending separate strategic review and work-order approval.",
        ],
        "safety": {
    ...
    certificate = {
        "schema_version": "1.0",
        "certificate_id": "sl-corpus-v4-fixed-scale2000-surface05-diagnostic-v1",
        "status": "DIAGNOSTIC_ONLY",
        "work_order_id": "0038",
        "corpus_id": config["data"]["corpus_id"],
        "fixed_text_sha256": config["data"]["fixed_text_sha256"],
        ...
        "selected_checkpoint_sha256": evaluation["checkpoint_sha256"],
        "classification": evaluation["classification"],
        "parameter_integrity_passed": training["parameter_integrity"]["only_surface05_changed"],
        "accepted_parent": "none",
        "promotion_eligible": False,
        "training_eligible": False,
        "checkpoint_accepted": False,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Work Order 0038 Surface05 diagnostic")
