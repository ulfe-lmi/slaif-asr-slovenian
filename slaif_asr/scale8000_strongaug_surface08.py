...
EXPERIMENT_ID = "surface08-scale8000-otf-strongaug-v1"
WORK_ORDER_ID = "0046"
POLICY_CONFIG_SHA256 = "588e6d04a445bd2a0b84fcda4dfd9e5274b0f1800e578a83a95e82fab8a430c9"
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config.get("experiment_id") != EXPERIMENT_ID:
        raise ValueError("unexpected StrongAug experiment identity")
    ...
    if sha256_file(policy_path) != POLICY_CONFIG_SHA256:
        raise ValueError("StrongAug policy config SHA256 mismatch")
    ...
    if config["model"].get("initialization") != "untouched_base":
        raise ValueError("StrongAug must start from the untouched base")
    if config["data"].get("training_source") != "scale8000_clean_otf_only":
        raise ValueError("StrongAug must use only scale-8000 clean OTF training")
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("StrongAug experiment config must be a JSON object")
...
def load_policy_for_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if sha256_file(path) != POLICY_CONFIG_SHA256:
        raise RuntimeError("StrongAug policy config SHA256 mismatch")
