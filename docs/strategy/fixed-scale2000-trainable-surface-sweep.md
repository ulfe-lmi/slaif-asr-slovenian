...
| `SURFACE_08_FULL_ENCODER` | decoder + joint + all encoder layers + proven `prompt_kernel`, with frontend and prompt identity frozen | Active exceptional boundary diagnostic under Work Order 0043 |
...
diagnostic to test whether full encoder exposure helps or overfits; it does not
make full-encoder training the default direction and does not authorize
Surface09.
